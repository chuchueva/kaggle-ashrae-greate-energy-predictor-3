...
train_data = 'train_site_0_meter_1.feather'
test_data = 'test_site_0_meter_1.feather'
result_to_update_file = 'late_model_03.csv'
result_file = 'late_model_04.csv'
model_file = 'model_04.pickle'
...
df_train_raw = ud.prepare_data_glb(df_train_raw, building, df_weather)

print('Training data is prepared, time %.0f sec' % (time.time() - start_time))

seasonality_models = dict()
building_list = [7, 14, 15, 31, 50, 60, 76, 82, 92, 95, 96, 97]
meter_type_list = [1]
df_train_raw['seasonality_model'] = np.nan
...
for building in building_list:
    ...
    for meter_type in meter_type_list:
        ...
        if len(df_sample_building) > 0:
            ...
            model, pred = ud.get_seasonality_model(df_sample_building)
            rmse = ud.get_error(df_sample_building['meter_reading'].values, pred)
...
regress_mask = np.invert(np.isnan(df_train_raw['seasonality_model']))
print('RMSLE simple: %.2f' % ud.get_error(df_train_raw.loc[regress_mask, 'meter_reading'].values,
                                          df_train_raw.loc[regress_mask, 'seasonality_model'].values))

df_train_raw['meter_reading_e1'] = df_train_raw['meter_reading'] - df_train_raw['seasonality_model']
df_train = ud.feature_engineering(df_train_raw)
...
df_train_raw.loc[regress_mask, 'meter_model'] = (df_train_raw.loc[regress_mask, 'seasonality_model'] +
                                                 df_train_raw.loc[regress_mask, 'residual_model']) / 2
df_train_raw.loc[np.invert(regress_mask), 'meter_model'] = df_train_raw.loc[np.invert(regress_mask), 'residual_model']

print('RMSLE regress: %.2f' % ud.get_error(df_train_raw.loc[regress_mask, 'meter_reading'].values,
                                           df_train_raw.loc[regress_mask, 'seasonality_model'].values))
print('RMSLE tree: %.2f' % ud.get_error(df_train_raw['meter_reading'].values, df_train_raw['residual_model'].values))
print('RMSLE total: %.2f' % ud.get_error(df_train_raw['meter_reading'].values, df_train_raw['meter_model'].values))
...
building = pd.read_csv(c.SOURCE_FOLDER + building_file)
df_test = ud.prepare_data_glb(df_test, building, df_weather)
df_test = ud.feature_engineering(df_test)
...
for site_id in range(1):
    ...
    for b in building_list:
        ...
        _, pred = ud.get_seasonality_model(df_test[mask],
                                           model['seasonality']['models']['building_%d_meter_%d' % (b, 1)])
    ...
    regress_mask = df_test['building_id'].isin(building_list)
    df_test.loc[regress_mask, 'meter_model'] = (df_test.loc[regress_mask, 'seasonality_model'] +
                                                df_test.loc[regress_mask, 'residual_model']) / 2
    df_test.loc[np.invert(regress_mask), 'meter_model'] = df_test.loc[np.invert(regress_mask), 'residual_model']
